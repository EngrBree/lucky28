# ✅ Corrected train_big_small.py with Integrated Real-Time Evaluation & F1 Metrics
...
def adaptive_retrain_loop(interval_minutes=60):
    ...
    scheduler1 = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.0008, steps_per_epoch=len(train_loader), epochs=50)
    scheduler2 = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.3, patience=5, verbose=True)
